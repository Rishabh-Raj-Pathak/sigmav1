...
export function getFundingRateHistory(marketToken: string, hours: number): FundingRateSnapshot[] {
...
export function insertOpportunity(o: Opportunity): number {
  const result = db().prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, 'active')
  `).run(o.tokenSymbol, o.longVenue, o.shortVenue, o.fundingSpread, o.entryPrice, o.estimatedApr, o.riskScore)
  return Number(result.lastInsertRowid)
}

export function getActiveOpportunities(): Opportunity[] {
  return db().prepare(`SELECT * FROM opportunities WHERE status = 'active' ORDER BY estimated_apr DESC`).all() as Opportunity[]
}

export function expireOldOpportunities(maxAgeMinutes: number): number {
  const cutoff = Math.floor(Date.now() / 1000) - maxAgeMinutes * 60
  return db().prepare(`UPDATE opportunities SET status = 'expired', expired_at = unixepoch() WHERE status = 'active' AND detected_at < ?`).run(cutoff).changes
}

export function markOpportunityTaken(id: number): void {
  db().prepare(`UPDATE opportunities SET status = 'taken' WHERE id = ?`).run(id)
}
...
export function getOpenTrades(): PaperTrade[] {
  return (db().prepare(`SELECT * FROM paper_trades WHERE status = 'open' ORDER BY opened_at DESC`).all() as Record<string, unknown>[]).map(mapTrade)
}

export function getAllTrades(limit = 50): PaperTrade[] {
  return (db().prepare(`SELECT * FROM paper_trades ORDER BY opened_at DESC LIMIT ?`).all(limit) as Record<string, unknown>[]).map(mapTrade)
}

export function getTradeById(id: number): PaperTrade | undefined {
  const row = db().prepare(`SELECT * FROM paper_trades WHERE id = ?`).get(id) as Record<string, unknown> | undefined
  return row ? mapTrade(row) : undefined
}

export function updateTradeState(id: number, currentPrice: number, fundingCollected: number, borrowingPaid: number, unrealizedPnl: number): void {
  db().prepare(`UPDATE paper_trades SET current_price = ?, funding_collected = ?, borrowing_paid = ?, unrealized_pnl = ? WHERE id = ?`)
    .run(currentPrice, fundingCollected, borrowingPaid, unrealizedPnl, id)
}

export function closeTrade(id: number, reason: string, realizedPnl: number, currentPrice: number): void {
  db().prepare(`UPDATE paper_trades SET status = 'closed', closed_at = unixepoch(), close_reason = ?, realized_pnl = ?, current_price = ? WHERE id = ?`)
...
export function insertVaultSnapshot(s: VaultSnapshot): void {
  ...
  db().prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(s.totalValueUsd, s.cashBalance, s.positionsValue, s.totalPnl, s.cumulativeFunding, s.numPositions, s.utilizationPct)
}
...
export function getVaultHistory(hours: number): VaultSnapshot[] {
  const since = Math.floor(Date.now() / 1000) - hours * 3600
  return (db().prepare(`SELECT * FROM vault_snapshots WHERE timestamp > ? ORDER BY timestamp ASC`).all(since) as Record<string, unknown>[]).map(mapVaultSnapshot)
}

export function getLatestVaultSnapshot(): VaultSnapshot | null {
  const row = db().prepare(`SELECT * FROM vault_snapshots ORDER BY timestamp DESC LIMIT 1`).get() as Record<string, unknown> | undefined
  return row ? mapVaultSnapshot(row) : null
}

export function getStrategyConfig(): StrategyConfig {
  const row = db().prepare(`SELECT * FROM strategy_config WHERE id = 1`).get() as Record<string, unknown>
  return {
...
export function updateStrategyConfig(c: StrategyConfig): void {
  ...
  `).run(c.minFundingSpread, c.maxPositionSizeUsd, c.maxTotalPositions,
...
export function insertSignal(s: Signal): void {
  db().prepare(`INSERT INTO signal_log (token_symbol, signal_type, action, reason, funding_spread, confidence, executed) VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .run(s.tokenSymbol, s.signalType, s.action, s.reason, s.fundingSpread || null, s.confidence || null, s.executed ? 1 : 0)
}

export function getRecentSignals(limit = 50): Signal[] {
  return db().prepare(`SELECT * FROM signal_log ORDER BY timestamp DESC LIMIT ?`).all(limit) as Signal[]
}

export function insertRebalanceEvent(e: RebalanceEvent): void {
  db().prepare(`INSERT INTO rebalance_events (action, trade_id, token_symbol, reason, old_size_usd, new_size_usd) VALUES (?, ?, ?, ?, ?, ?)`)
    .run(e.action, e.tradeId || null, e.tokenSymbol, e.reason, e.oldSizeUsd || null, e.newSizeUsd || null)
}

export function getRebalanceHistory(limit = 50): RebalanceEvent[] {
  return db().prepare(`SELECT * FROM rebalance_events ORDER BY timestamp DESC LIMIT ?`).all(limit) as RebalanceEvent[]
}
